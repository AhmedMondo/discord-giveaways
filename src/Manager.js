...
class GiveawaysManager extends EventEmitter {
    ...
    generateMainEmbed(giveaway, lastChanceEnabled = false) {
        const embed = new Discord.MessageEmbed()
            ...
            .setDescription(
                giveaway.isDrop
                    ...
                    : (
                        ...
                        ).replace('{longtimestamp}' , giveaway.endAt === Infinity ? '`NEVER`' : `<t:${Math.round(giveaway.endAt / 1000)}>`) */
                         
                        /* .replace('{durationgiveaway}' , giveaway.remainingTimeText) */
                    ) +
                    giveaway.remainingTimeText.replace(
                        '{timestamp}',
                        giveaway.endAt === Infinity ? '`NEVER`' : `<t:${Math.round(giveaway.endAt / 1000)}:R>`
                    ).replace('{longtimestamp}' , giveaway.endAt === Infinity ? '`NEVER`' : `<t:${Math.round(giveaway.endAt / 1000)}>`)
                     +
            ...
            .setThumbnail(giveaway.messages.embedThumbnail)
            .setImage(giveaway.messages.embedBanner);
